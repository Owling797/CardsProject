...
void GameRules::giveMore(Player *x) // додать игроку до 6 карт
{
    ...
    srand(time(0));
    while (x->curDeck.getNumber() < this->deckType / this->gamers && main_Deck.number)
    {
...
void UserInterface::results(int winner, int looser)
{
    cout << "\033[2J"; // очистка терминала
    cout << "RESULTS:" << endl
         << "Winner is: Player #" << winner << endl;
    cout << "Looser is: #" << looser << endl;
}
/////////////
...
////////////////////////
GeneralClass::GeneralClass(int _gm, int _ps, int _j, int _v)
{
    gm = _gm;
    ps = _ps;
    j = _j;
...
void GeneralClass::gamePlay()
{
    ...
    Game CurGame(type, num, gm, ps, j, v); // создали объект класса правил (весь)
    ...
    UI.results(CurGame.winnersID, cur_player->getIndex());
    /*cout << "\033[2J"; // очистка терминала
    cout << "RESULTS:" << endl
